...
if $plot; then
  ...
  for folder in "$output_directory"*; do
      ...
      srun -n 1 python helpers_py/plot3d_pointCloud.py --in-file "$root_path" &
      echo "Plot '$folder' done!!"
  done
  wait
else
  echo "Plots not created. Set 'plot=true' to create plots."
fi
echo "All runs completed. Check the output directory: $output_directory"
echo "End of script."